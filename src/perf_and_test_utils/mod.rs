...
/// by selecting a random value in [1, occs[c]], where occc[c] is the number of occurrences of c. 
pub fn gen_select_queries<T: Unsigned + Into<usize> + Copy>(n_queries: usize, s: &[T]) -> Vec<(usize, T)> {
    let mut rng = rand::thread_rng();
